...
namespace game_engine::ecs
{
    struct registry
    {
        ...
        {
            for (auto& [_, a] : _components) { GAMEENGINE_DELETE(a); }
            _components.clear();
            _signatures.clear();
            _nextid = INVALID_ID;
        }

        GAMEENGINE_INLINE entityid add_entity()
        {
        ...
        GAMEENGINE_INLINE void destroy(entityid e)
        {
        ...
        GAMEENGINE_INLINE bool is_alive(entityid e)
        {
            return _signatures.count(e);
        }

        template <typename T>
        GAMEENGINE_INLINE entitylist view()
        {
            entitylist list;
        ...
        template <typename T>
        GAMEENGINE_INLINE T& get_component(entityid e)
        {
            GAMEENGINE_ASSERT(_signatures.count(e) && "out of range!");
            return get_component_array<T>()->get(e);
        }

        template <typename T, typename... Args>
        GAMEENGINE_INLINE T& add_component(entityid e, Args&&...args)
        {
            GAMEENGINE_ASSERT(_signatures.count(e) && "out of range!");
        ...
        template <typename T>
        GAMEENGINE_INLINE void remove_component(entityid e)
        {
        ...
        template <typename T>
        GAMEENGINE_INLINE bool has_component(entityid e)
        {
        ...
        template <typename T>
        GAMEENGINE_INLINE component_array<T>* get_component_array()
        {
